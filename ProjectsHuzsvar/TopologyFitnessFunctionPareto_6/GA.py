...
def func(X):
	...
	f.close()
	cmd = './isoFF.out ' + Network + ' Topology_Demand input.csv'
	#print(cmd)
...
for j in range(1):
	...
	print(convergence)
	print(solution)
	g = open("results_"+str(ActualNetwork)+"_Demand_"+str(multipliers[j])+"_Convergence.txt", "w")
	g.write(str(convergence))
	g.close
	e = open("results_"+str(ActualNetwork)+"_Demand_"+str(multipliers[j])+"_Results.txt", "w")
